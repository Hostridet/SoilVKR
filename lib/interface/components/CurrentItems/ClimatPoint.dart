...
class ClimatItem extends StatefulWidget {
  final PointWithRoute args;
  const ClimatItem({Key? key, required this.args}) : super(key: key);

  @override
  State<ClimatItem> createState() => _AnimalItemState();
}

class _AnimalItemState extends State<ClimatItem> {
      ...
      child: Scaffold(
        ...
        body: RepositoryProvider(
          ...
          child: BlocProvider<PointBloc>(
            create: (context) => PointBloc(RepositoryProvider.of<PointRepository>(context))..add(PointGetClimatEvent(widget.args.point.id)),
            child: BlocBuilder<PointBloc, PointState>(
              builder: (context, state) {
                if (state is PointLoadingState) {
                  return Center(child: CircularProgressIndicator());
                }
                if (state is PointLoadedClimatState) {
                  return Column(
                    children: [
                      Expanded(
                                  ...
                                  onTap: () {
                                    Navigator.of(context).pushReplacementNamed('/home/book/climat',
                                        arguments: ItemWithRoute(
                                            id: state.foundationList[index].id, route: '/home/points/climat', point: widget.args));
                                  },
                                ),
                              );
                            }),
